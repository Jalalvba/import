...
**Memory:** `vercel.json`'s `functions["api/index.py"]` requests `"memory": 2048` (MB), up from the platform default (1024 MB), for headroom against a larger-than-tested file (e.g. a full year of data). This was previously set to `3009` (the documented absolute maximum configurable value), but a deployment on this project's Hobby plan clamped/rejected it — `2048` is the actual usable ceiling on Hobby, confirmed by checking the effective value after deploying (Vercel dashboard → Project → Functions, or `vercel inspect`) rather than assuming the requested number took effect. **Do not bump this back toward 3009 without first confirming the plan has been upgraded past Hobby** — otherwise it's a config value Vercel will silently clamp again. This 3009→2048 clamp is a historical, point-in-time observation from one deployment, not something re-verifiable from the repo alone — Vercel's platform limits can change independently of this project. If this value is ever revisited (bumping it up, chasing a memory-related failure, etc.), re-confirm the actual effective ceiling with `vercel inspect` (or the dashboard) rather than trusting this note as still-current fact. Together, `maxDuration: 180` + `memory: 2048` are the practical ceiling for what "too big to run on Vercel" means in this project — a file that trips the `hard_fail` size tier (see [File Size Handling](#file-size-handling--heavy-excel-exports)) is explicitly meant to be run locally instead of pushed against these limits.
...
├── scripts/
│   ├── gdrive_auth_check.py      # optional manual debugging tool — Drive auth + listing only (NOT a pytest test; see tests/ below)
│   └── gdrive_download_check.py  # optional manual debugging tool — Drive download + signature check (NOT a pytest test)
├── tests/                   # pytest suite — mocks everything, no real Drive/Mongo needed (see "Setup & Running" above)
